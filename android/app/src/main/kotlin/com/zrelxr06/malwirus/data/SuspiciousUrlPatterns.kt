...
object SuspiciousUrlPatterns {
    ...
    val SUSPICIOUS_DOMAINS = listOf(
        ...
        "titan.cf", "br-icloud.com.br", "pinoyinsta.com", "discord.com",
        "blogspot.cf", "instacash", "dito"
    )
     ...
     */
    val SUSPICIOUS_TLDS = listOf("tk", "ml", "ga", "cf", "gq", "ly", "pw", "cc", "su", "xyz", "top", "club", "icu", "br")
    ...
    fun isUrlSuspicious(url: String): Pair<Boolean, String?> {
        ...
        if (domain != null) {
            ...
            for (suspiciousDomain in SUSPICIOUS_DOMAINS) {
                ...
                if (domain == suspiciousDomain || 
                    ...
                    Log.d(TAG, "Match found! Domain $domain matches suspicious domain $suspiciousDomain")
        ...
        for (suspiciousDomain in SUSPICIOUS_DOMAINS) {
            if (lowercaseUrl == suspiciousDomain || lowercaseUrl == "www.$suspiciousDomain" || 
                ...
                Log.d(TAG, "Match found! URL $lowercaseUrl matches suspicious domain format with $suspiciousDomain")
    ...
    fun extractUrls(message: String): List<String> {
        ...
        val obfuscationReplacements = mutableListOf<Triple<String, String, Int>>() // original, replacement, startPosition
        ...
        while (obfuscatedDomainMatcher.find()) {
            ...
            val normalizedDomain = domainFirst + "." + domainSecond + domainRest.replace("\\s", "") + path
        ...
        for (url in urls) {
            val urlToReplace = if (url.startsWith("http://")) url.substring(7) else if (url.startsWith("https://")) url.substring(8) else url
            messageWithoutUrls = messageWithoutUrls.replace(urlToReplace, " ".repeat(urlToReplace.length))
        }
        ...
        val shortenerDomains = SUSPICIOUS_DOMAINS.filter { it.length < 15 } // Include more domains
        ...
        val shortenerMatcher = Pattern.compile(shortenerPattern.toString(), Pattern.CASE_INSENSITIVE).matcher(messageWithoutUrls)
        ...
        val spaceInUrlPattern = Pattern.compile("(https?://[^\\s]+)\\s+(/[^\\s]*)", Pattern.CASE_INSENSITIVE)
        ...
        val barePattern = Pattern.compile("\\b([a-zA-Z0-9][a-zA-Z0-9-]*\\.(${SUSPICIOUS_TLDS.joinToString("|")}))(\\b|$)", Pattern.CASE_INSENSITIVE)
        ...
        Log.d(TAG, "Extracted ${uniqueUrls.size} unique URLs from message")
        return uniqueUrls
    }
}
